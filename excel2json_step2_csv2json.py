...
def create_nested_record(base: List[str], col_to_path: Dict[int, List[Tuple[str, bool]]],
...
def merge_yaml_dicts(yaml_str1: str, yaml_str2: str) -> Dict[str, Any]:
    ...
    - 多段の配列の場合、最も深い階層でマージを行う。
    """
    combined_yaml = yaml_str1 + "\n---\n" + yaml_str2  # YAML ドキュメントを連結
    merged_dict = {}
        ...
        for key, value in doc.items():
            if key in merged_dict:
                ...
                if isinstance(merged_dict[key], list):
                    ...
                    merged_dict[key] = value  # 上書き
            else:
                merged_dict[key] = value  # 新規追加

    return merged_dict

def process_record_group(rows: List[List[str]], col_to_path: Dict[int, List[Tuple[str, bool]]], num_cols: int) -> Dict[str, Any]:
    """
    １レコードに属する複数行（最初の行は新規レコード、以降は "*" マーカーの継続行）
    を受け取り、YAML マージを行って統合された辞書を返す。
    """
    record: Dict[str, Any] = {}

    # まず、新規レコード行（マーカーが空文字）の処理
    base_record = create_nested_record(rows[0], col_to_path, num_cols)
    base_yaml = yaml.dump(base_record, allow_unicode=True)
    #print(f"test_record (YAML):\n{base_yaml}")
